...
> {
  ...
  for (let i = 0; i < batches.length; i += MAX_CONCURRENT_GEMINI) {
    ...
    console.log(
      `[digest] Scoring progress: ${Math.min(
...
async function summarizeArticles(
  ...
  for (let i = 0; i < batches.length; i += MAX_CONCURRENT_GEMINI) {
    ...
    await Promise.all(promises);
    console.log(
      `[digest] Summary progress: ${Math.min(
        i + MAX_CONCURRENT_GEMINI,
        batches.length
      )}/${batches.length} batches`
    );
...
async function generateHighlights(
  ...
  const articleList = articles
    ...
    .map(
      (a, i) =>
        `${i + 1}. [${a.category}] ${a.titleZh || a.title} — ${a.summary.slice(
          0,
          100
        )}`
    )
...
直接返回纯文本总结，不要 JSON，不要 markdown 格式。`;
  ...
  try {
    ...
    console.warn(
      `[digest] Highlights generation failed: ${
        error instanceof Error ? error.message : String(error)
      }`
    );
    return "";
...
function generateDigestReport(
  ...
  if (articles.length >= 3) {
    ...
    for (let i = 0; i < Math.min(3, articles.length); i++) {
      ...
      report += `[${a.title}](${a.link}) — ${a.sourceName} · ${humanizeTime(
        a.pubDate
  ...
  } | 扫描 ${stats.successFeeds} 源 → 获取 ${stats.totalArticles} 篇 → 精选 ${
    articles.length
  } 篇*\n`;
...
async function main(): Promise<void> {
  ...
  if (!geminiApiKey && !openaiApiKey) {
    console.error(
      "[digest] Error: Missing API key. Set GEMINI_API_KEY and/or OPENAI_API_KEY."
    );
  ...
  console.log(
    `[digest] AI provider: ${
      geminiApiKey ? "Gemini (primary)" : "OpenAI-compatible (primary)"
    }`
  );
  if (openaiApiKey) {
    const resolvedBase = (
  ...
  console.log(
    `[digest] Top ${topN} articles selected (score range: ${
      topArticles[topArticles.length - 1]?.totalScore || 0
    } - ${topArticles[0]?.totalScore || 0})`
  );
